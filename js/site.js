...
function start () {
	...
	if (storage.works) {
		...
		$.each(works_json, function (i, val) {
			$("#works").append('<li class="span3"><a href="http://'+val.url+'" title="'+val.nombre+' ('+val.fecha+')" rel="#'+val.tag+'"><img src="/images/works/thumb/'+val.tag+'.jpg" alt=""></a><span>'+val.nombre+' ('+val.fecha+')</span></li>')
